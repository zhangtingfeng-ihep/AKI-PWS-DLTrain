...
SLICE_DIR = r"F:\Sample9\PythonProject\PWS\image"  # Path to slice data
LABEL_DIR = r"F:\Sample9\PythonProject\PWS\label"  # Path to label data
OUTPUT_DIR = r"F:\Sample9\PythonProject\PWS\TransUnet\Model_Output"  # Output directory
if not os.path.exists(OUTPUT_DIR):
    os.makedirs(OUTPUT_DIR)

PATCH_SIZE = 32  # Patch size
PATCHES_PER_IMAGE = 32  # Number of patches per image
BATCH_SIZE = 16  # Batch size
EPOCHS = 50  # Number of epochs
NUM_CLASSES = 2  # Number of classes (kidney=1, background=0)
...
def load_and_preprocess_data():
    """Load and preprocess slice and label data."""
...
def augment_patch(patch, mask, seed_val):
    """Apply data augmentation to image patch and mask."""
...
class CombinedLoss(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        target_pred = torch.softmax(outputs, dim=1)[:, 1, :, :]
        target_target = (targets == 1).float()
        intersection = (target_pred * target_target).sum()
        dice = 1 - (2. * intersection + 1) / (target_pred.sum() + target_target.sum() + 1)
        return ce + self.dice_weight * dice

def train_TransUNet_model():
    ...
    model = TransUNet(in_channels=1, out_channels=NUM_CLASSES, img_size=PATCH_SIZE)
...
def compute_metrics(pred, target, num_classes=2):
    """Compute IoU, recall, F1, Dice, and class accuracies."""
...
if __name__ == "__main__":
    try:
        ...
        if torch.cuda.is_available():
            ...
            print("Warning: No GPU detected, using CPU for training, which may be slow!")

        model, test_slice_files, test_label_files = train_TransUNet_model()
