...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=2048):  # 添加 img_size 参数
        super(Embeddings, self).__init__()
        self.config = config
        self.hybrid = None
        downsample_factor = 32  # ResNet 的下采样倍数
        feature_size = img_size // downsample_factor  # 特征图大小
        self.n_patches = feature_size ** 2  # 动态计算 patch 数量
    ...
    def forward(self, x):
        ...
        # 添加检查，确保 patch 数量匹配
        assert x.size(1) == self.n_patches, f"Expected {self.n_patches} patches, but got {x.size(1)}"
...
class TransformerModel(nn.Module):
    def __init__(self, config, img_size, vis):  # 添加 img_size 参数
        super(TransformerModel, self).__init__()
        self.embeddings = Embeddings(config, img_size, in_channels=2048)
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=2, zero_head=False, vis=False):
        ...
        self.transformer = TransformerModel(config, img_size, vis=vis)  # 传递 img_size
...
def get_transunet_config(n_classes=2, patch_size=8):
    config = {}
    config['n_classes'] = n_classes  # 修改为2
...
class TransUNet(nn.Module):
    def __init__(self, in_channels=1, out_channels=2, img_size=128):  # img_size=128
