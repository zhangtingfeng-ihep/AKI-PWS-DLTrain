...
OUTPUT_DIR = r"F:\Sample9\PythonProject\PWS\swin-unet\Model_Output2"
...
# Configuration class for Swin-Unet
class Config:
    class DATA:
        IMG_SIZE = 32
    class MODEL:
...
class SwinUNet(nn.Module):
    def __init__(self, config, img_size=32, num_classes=NUM_CLASSES):
...
def load_and_preprocess_data():
    ...
    assert len(slice_files) == len(label_files), "Mismatch between slice and label file counts"
...
def augment_patch(patch, mask, seed_val):
    ...
    if HORIZONTAL_FLIP and random.random() > 0.5:
        ...
        mask = cv2.flip(patch, 1)
    if VERTICAL_FLIP and random.random() > 0.5:
        patch = cv2.flip(patch, 0)
        mask = cv2.flip(patch, 0)
