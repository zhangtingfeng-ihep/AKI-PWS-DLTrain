...
# 1. Set random seeds to ensure reproducibility of results
seed = 42
np.random.seed(seed)
...
OUTPUT_DIR = r"F:\Sample9\U-net\Model_Output4"
...
class UNet(nn.Module):
    ...
    def __init__(self, in_channels=1, out_channels=NUM_CLASSES):
        ...
        self.dec4 = DoubleConv(1024, 512)  # 512 + 512 = 1024 (concat)

        self.upconv3 = nn.ConvTranspose2d(512, 256, kernel_size=2, stride=2)
        self.dec3 = DoubleConv(512, 256)  # 256 + 256 = 512 (concat)

        self.upconv2 = nn.ConvTranspose2d(256, 128, kernel_size=2, stride=2)
        self.dec2 = DoubleConv(256, 128)  # 128 + 128 = 256 (concat)

        self.upconv1 = nn.ConvTranspose2d(128, 64, kernel_size=2, stride=2)
        self.dec1 = DoubleConv(128, 64)  # 64 + 64 = 128 (concat)
...
def load_and_preprocess_data():
    ...
    #Retrieve all slice and label files
    ...
    #Confirm that the number of files matches
    assert len(slice_files) == len(label_files), "The number of slices and label files does not match"
    print(f"find {len(slice_files)} pairs slice and label files")

    return slice_files, label_files


# 5. Data standardization function
...
class KidneySegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask_tensor = torch.from_numpy(mask_patch).long() - 1  # 1,2,3 -> 0,1,2
...
class EarlyStopping:
    ...
    def __call__(self, val_loss, model, path):
        if val_loss < self.best_loss - self.min_delta:
            ...
            if self.verbose:
                print(f'Verify the reduction of losses and save the model to {path}')
        else:
            self.counter += 1
            if self.verbose:
...
def train_model(model, train_loader, val_loader, criterion, optimizer, device, num_epochs=EPOCHS):
    ...
    for epoch in range(num_epochs):
        ...
        print(f"Epoch {epoch + 1}/{num_epochs}, "
              ...
              f"Class 0 Acc: {val_class_acc[0]:.4f}, Class 1 Acc: {val_class_acc[1]:.4f}, Class 2 Acc: {val_class_acc[2]:.4f}, "
              f"Val Class0 IoU: {val_class_iou[0]:.4f}, Val Class1 IoU: {val_class_iou[1]:.4f}, Val Class2 IoU: {val_class_iou[2]:.4f}")
...
def train_unet_model():
    ...
    print(f"Dynamically calculated category weights: {class_weights.tolist()}")
    ...
    plt.title('changes in Learning rate')
    plt.xlabel('Epoch')
    ...
    print("Model training completed! The best model has been saved.")
    return model, test_slice_files, test_label_files


# 12. Evaluate the model function
def evaluate_model(model, test_slice_files, test_label_files, device):
    ...
    with open(os.path.join(OUTPUT_DIR, 'test_results.txt'), 'w') as f:
        f.write(f"Test set loss: {avg_test_loss:.4f}\n")
...
def compute_metrics(pred, target, num_classes=3):
    ...
    class_iou = []
    for c in range(num_classes):
        iou_c = jaccard_score(target_flat, pred_flat, average=None, labels=[c], zero_division=0)[0] if (
                                                                                                                   target_flat == c).sum() > 0 else 0.0
...
def compute_class_iou(pred, target, class_idx):
...
def visualize_predictions(model, test_slice_files, test_label_files, device, num_samples=3):
    ...
    print("The visualization of the prediction results has been completed, and the image has been saved.")


# 14. Conduct training and evaluation
if __name__ == "__main__":
    try:
        ...
        if torch.cuda.is_available():
            ...
            print(f"available GPU number: {torch.cuda.device_count()}")


        else:
            device = torch.device("cpu")
            print("Warning: GPU not detected, CPU will be used for training, which may be slow!")
        ...
        print("The final model has been saved to:", os.path.join(OUTPUT_DIR, 'final_model.pth'))
