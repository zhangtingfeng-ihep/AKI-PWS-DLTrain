...
class AxialAttention_wopos(nn.Module):
    ...
    def forward(self, x):
        ...
        qk = torch.einsum('bgci, bgcj->bgij', q, k)
        stacked_similarity = self.bn_similarity(qk).view(N * W, 1, self.groups, H, H).sum(dim=1).view(N * W, self.groups, H, H)
        similarity = F.softmax(stacked_similarity, dim=3)
        sv = torch.einsum('bgij,bgcj->bgci', similarity, v)
...
class medt_net(nn.Module):
    def __init__(self, block, block_2, layers, num_classes, groups=8, s=0.125, img_size=256, imgchan=1):
        ...
        self.inplanes = int(512 * s * block_2.expansion)
        self.maxpool_p4 = nn.MaxPool2d(2, stride=2)
        self.encoder4p = self._make_layer(block_2, int(1024 * s), layers[3], kernel_size=(img_size_p // 16))
        self.decoder1_p = nn.Conv2d(int(1024 * s * block_2.expansion), int(1024 * s * block_2.expansion), kernel_size=3, stride=1, padding=1)
        self.decoder2_p = nn.Conv2d(int(1024 * s * block_2.expansion), int(512 * s * block_2.expansion), kernel_size=3, stride=1, padding=1)
        self.decoder3_p = nn.Conv2d(int(512 * s * block_2.expansion), int(256 * s * block_2.expansion), kernel_size=3, stride=1, padding=1)
        self.decoder4_p = nn.Conv2d(int(256 * s * block_2.expansion), int(128 * s * block_2.expansion), kernel_size=3, stride=1, padding=1)
        self.decoder5_p = nn.Conv2d(int(128 * s * block_2.expansion), int(128 * s * block_2.expansion), kernel_size=3, stride=1, padding=1)
        ...
        self.decoder5 = nn.Conv2d(int(256 * s * block.expansion), int(128 * s * block.expansion), kernel_size=3, stride=1, padding=1)
    ...
    def _make_layer(self, block, planes, blocks, kernel_size=16, stride=1, dilate=False):
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, groups=8, base_width=64, dilation=previous_dilation, norm_layer=norm_layer, kernel_size=kernel_size))
        self.inplanes = planes * block.expansion
        if stride != 1:
            kernel_size = kernel_size // 2
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, planes, groups=8, base_width=64, dilation=self.dilation, norm_layer=norm_layer, kernel_size=kernel_size))
    ...
    def _forward_impl(self, x):
        ...
        for i in range(0, 4):
            for j in range(0, 4):
                ...
                x_p = F.interpolate(x_p, size=(64, 64), mode='bilinear', align_corners=False)
...
def MedT(img_size=256, imgchan=1, num_classes=3):
    model = medt_net(AxialBlock_dynamic, AxialBlock_wopos, [1, 2, 4, 1], num_classes=num_classes, s=0.125, img_size=img_size, imgchan=imgchan)
...
OUTPUT_DIR = r"F:\Sample9\MedT\Model_Output4"
...
class KidneySegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        assert mask_tensor.min() >= 0 and mask_tensor.max() <= 2, f"Label value out of range: min={mask_tensor.min()}, max={mask_tensor.max()}"
...
def load_and_preprocess_data():
    ...
    assert len(slice_files) == len(label_files), "Mismatch between slice and label files"
    print(f"Found {len(slice_files)} slice and label file pairs")
...
def train_medt_model():
    ...
    train_val_slice_files, test_slice_files, train_val_label_files, test_label_files = train_test_split(slice_files, label_files, test_size=TEST_SPLIT, random_state=seed)
    train_slice_files, val_slice_files, train_label_files, val_label_files = train_test_split(train_val_slice_files, train_val_label_files, test_size=VALIDATION_SPLIT / (1 - TEST_SPLIT), random_state=seed)
    total_pixels = 0
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"Epoch {epoch + 1}/{EPOCHS}, Train Loss: {avg_train_loss:.4f}, Train Acc: {train_acc:.4f}, Val Loss: {avg_val_loss:.4f}, Val Acc: {val_acc:.4f}, Val IoU: {val_iou:.4f}, Val Recall: {val_recall:.4f}, Val F1: {val_f1:.4f}, Val Dice: {val_dice:.4f}")
    ...
    with open(os.path.join(OUTPUT_DIR, 'metrics_history.txt'), 'w', encoding='utf-8') as f:
        ...
        for epoch in range(len(history['train_loss'])):
            f.write(f"{epoch+1}\t{history['train_loss'][epoch]:.4f}\t{history['val_loss'][epoch]:.4f}\t{history['train_acc'][epoch]:.4f}\t{history['val_acc'][epoch]:.4f}\t{history['val_iou'][epoch]:.4f}\t{history['val_recall'][epoch]:.4f}\t{history['val_f1'][epoch]:.4f}\t{history['val_dice'][epoch]:.4f}\t{history['val_class0_acc'][epoch]:.4f}\t{history['val_class1_acc'][epoch]:.4f}\t{history['val_class2_acc'][epoch]:.4f}\t{history['val_class0_iou'][epoch]:.4f}\t{history['val_class1_iou'][epoch]:.4f}\t{history['val_class2_iou'][epoch]:.4f}\t{history['learning_rate'][epoch]:.6f}\n")
